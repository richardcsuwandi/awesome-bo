...
Skip the GP book if Distill already made sense. Then Garnett's book for theory, or Bayesian Optimization in Action for a Python walkthrough (under Books).

Then a library (under Software). Ax and Vizier are services: you declare the search space and the metric, and they run the loop. Start with Ax unless you already use Google's stack. BoTorch is the PyTorch toolkit under Ax. You write that loop yourself when you are implementing a method, or changing the surrogate or the acquisition.
